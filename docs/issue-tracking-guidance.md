...
All epics and stories live in GitHub issues in `YourTechBudStudio/Isagi`. Use issue numbers and URLs as identifiers, and `gh` or `gh api` to manage them. No separate local issue copies, Projects, or milestones by default.

Titles are bare product outcomes with no type prefix; suffix a candidate epic with `(candidate)`.
...
Most operations are ordinary `gh issue create`, `gh issue edit --parent`, `gh issue comment`, and `gh issue close`. Dependencies have no `gh issue` surface and need the REST API, which takes the blocker's database ID as a typed integer field, not its issue number:

```
gh api repos/YourTechBudStudio/Isagi/issues/<issue>/dependencies/blocked_by -F issue_id=$(gh api repos/YourTechBudStudio/Isagi/issues/<blocker> --jq .id)
gh api repos/YourTechBudStudio/Isagi/issues/<issue>/dependencies/blocked_by --jq '.[].number'
```

Pass bodies and comments with `--body-file`; inline `--body` mangles multi-line Markdown.
