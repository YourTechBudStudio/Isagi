...
This document defines how Isagi's epics and stories are represented, retrieved, and amended. It is the repository-specific issue-tracking contract used by the `shaping-issues` skill. The filename is independent of the configured tracker; this document records Isagi's current mapping.

Isagi currently stores epics and stories in GitHub Issues on `YourTechBudStudio/Isagi`. They are not stored in this repository. This document is the mapping; GitHub is the source of truth.

Read this document before retrieving, creating, or amending any epic or story. When a task begins with an issue number, follow [Retrieval](#retrieval) before doing anything else.

## Object Model

| Concept                        | GitHub representation                                                                                                                                                 |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Candidate epic                 | An issue whose title begins with `[epic] `, is labeled `Type: Epic` and `Epic: Candidate`, and has no stories                                                         |
| Committed epic                 | An issue whose title begins with `[epic] ` and is labeled `Type: Epic` and `Epic: Committed`                                                                          |
| Story                          | An issue whose title begins with `[story] `, is labeled `Type: Story` and exactly one `Story:*` label, and is attached as a **sub-issue** of its committed epic issue |
| Story kind                     | `Story: Exploration`, `Story: Implementation`, or `Story: Release`                                                                                                    |
| Epic ↔ story navigation        | GitHub's native sub-issue relationship                                                                                                                                |
| Relationship between epics     | A plain issue reference in the body (`Blocked by #12` or `Related to #12`)                                                                                            |
...
| Label                   | Meaning                                                                                                                                                            |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Type: Epic`            | The issue is an epic.                                                                                                                                              |
| `Epic: Candidate`       | The epic is an uncommitted, storyless direction preserved for future shaping.                                                                                      |
| `Epic: Committed`       | The epic is hardened and may have stories.                                                                                                                         |
| `Type: Story`           | The issue is a story under a committed epic.                                                                                                                       |
| `Story: Exploration`    | The story reduces a consequential uncertainty and applies its conclusions to the epic and downstream stories.                                                      |
...
GitHub's native Milestone feature is reserved for **release trains**: it groups epic issues into the versions they ship in. That is a human release-planning decision, made separately from shaping.

Never create, close, rename, or assign a native GitHub Milestone as part of shaping or persisting an epic or story. Do it only when explicitly asked.
...
3. If it is a story, read its committed parent epic the same way, comments included.
4. List the epic's other stories for sibling context: `gh api repos/YourTechBudStudio/Isagi/issues/<epic>/sub_issues --jq '.[] | "\(.number) \(.title) [\(.state)]"'`.
...
Both epic kinds use `[epic] <product outcome>`. The lowercase prefix is mandatory. Refine the title when understanding changes; record a material rename in an amendment.

### Story Issue

- **Why this exists** — how the story advances its epic.
...
- **Feeds** — the epic or downstream stories that will consume the conclusions, as issue references.
...
Every kind of revision is an amendment: reshaping an epic, changing acceptance criteria, splitting or merging shaped objects, recording an exploration's conclusions, or parking work.

### Candidate Activation

Use the candidate as input to fresh shaping. Once hardened, post an amendment with the activation reasoning and consolidated committed state, refine the title if needed, replace `Epic: Candidate` with `Epic: Committed`, then create its stories. Never attach a story to a candidate.
...
Post the conclusions as an amendment comment on the exploration story, then post amendment comments on the parent epic and on every affected downstream story. Reconciliation may confirm, revise, add, remove, split, or merge stories, or reshape the epic itself. Close the exploration story only after those amendments exist.
...
- **Open candidate epic** — uncommitted and available for future shaping.
- **Open committed epic or story** — active or parked.
- **Closed committed epic or story** — done and not expected to be revisited.
...
Preview before publishing. Present a candidate at summary depth. Present a committed epic and its stories with titles, story kinds, body shapes, and acceptance criteria in substance. Verbatim body text is not required for approval.

Publish a candidate by itself. Publish a committed epic and its stories in dependency order so parentage can be set at creation time.

1. Create the epic issue first.
2. Create each story one at a time, attached to the epic.
3. Report the created issue numbers back.

## Commands

Create a candidate epic:

```
gh issue create --title "[epic] <outcome>" --label "Type: Epic" --label "Epic: Candidate" --body-file <file>
```

Create a committed epic:

```
gh issue create --title "[epic] <outcome>" --label "Type: Epic" --label "Epic: Committed" --body-file <file>
```

Create a story under it:

```
gh issue create --title "[story] <outcome>" --label "Type: Story" --label "Story: Implementation" --parent <epic> --body-file <file>
```

Attach or move an existing story:

```
gh issue edit <story> --parent <epic>
gh issue edit <epic> --add-sub-issue <story>
gh issue edit <epic> --remove-sub-issue <story>
```
...
```
gh issue edit <candidate> --remove-label "Epic: Candidate" --add-label "Epic: Committed"
```
...
```
gh issue list --label "Type: Epic" --label "Epic: Candidate" --state open
gh issue list --label "Type: Epic" --label "Epic: Committed" --state all
gh issue list --label "Type: Story" --label "Story: Exploration" --state open
gh api repos/YourTechBudStudio/Isagi/issues/<epic>/sub_issues
```
...
Splitting a story creates the new stories under the same epic, then records the split as an amendment comment on the original. If the original no longer has an outcome of its own, close it with `Won't Fix` and point at the replacements.
...
In both cases the epic gets an amendment when its story set changes materially.
