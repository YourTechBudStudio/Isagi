...
`activeContext` is frontend-owned live workspace selection persisted by the runtime for restart restoration. The runtime may validate and store it through dedicated active-context persistence APIs. It is not part of the workspace snapshot and is not runtime operational scope.
