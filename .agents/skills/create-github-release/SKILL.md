...
Draft a GitHub release for **Isagi**. The version comes from the root `package.json` — the user passes no arguments. Follow the steps in order. Don't write release notes until prerequisites pass and the user has confirmed.
...
Load the `design-system` skill and use its "Voice & Copywriting (Isagi tone)" section. Tone in one sentence: witty, nerdy, comedic-villain-but-friendly, developer humor, useful first, one punchline per surface, no emojis.

Skeleton (the title uses `<version>` without the `v` prefix):

```markdown
# Isagi <version> — <short evocative title>

<One-line opener in Isagi voice.>
...
Resolve the target SHA with `git rev-parse HEAD`. Write the notes to a temp file (so Markdown survives), then:
