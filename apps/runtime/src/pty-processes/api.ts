  ...
  registerApiEndpoint(fastify, apiEndpoints.surfaces.launchAgentSession, {
    handle: (input, _context, params) =>
      Effect.gen(function* () {
        ...
        return yield* service.launch({ worktreeId: params.worktreeId, harness: input.harness });
      }),
  ...
  registerApiEndpoint(fastify, apiEndpoints.surfaces.launchTerminalSession, {
    handle: (_input, _context, params) =>
      Effect.gen(function* () {
        ...
        return yield* service.launch({ worktreeId: params.worktreeId });
      }),
    mapError: (error, context) => toSessionLaunchApiError(error, context),
    run,
  });
...
function registerSessionAttachRoute(
  ...
  fastify.get(
    ...
    (socket, request) => {
      ...
      void run(input.resolveProcessId(sessionId).pipe(Effect.either))
...
function toSessionLaunchApiError(error: unknown, context: ApiRouteContext): ApiError {
  if (
    ...
    /Worktree .* was not found/.test(error.cause.message)
  ) {
    return {
  ...
  console.error(
    `[runtime] Unhandled session API handler error during ${context.endpointId}`,
    error,
  );
  return {
