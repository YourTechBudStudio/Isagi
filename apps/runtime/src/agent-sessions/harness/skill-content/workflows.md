...
- Read `ctx.invocation.kind` when a failed step needs retry-specific recovery. `normal` is ordinary dispatch; `retry` marks only the first replay of the failed leaf. The saved event is preserved except when explicit Retry reconciles an agent-response step against the latest turn in the same durable agent session.
...
- Root Retry repairs the failed workflow branch using the latest verified workflow artifacts: failed child leaves become ready, failed ancestors return to their recorded workflow joins, and both kinds of failed run receive current artifact pins before recovery; completed siblings remain terminal on their existing pins. The persisted state is replayed into the updated code, so keep state compatible across workflow updates. Explicit agent-response recovery may deliver a later turn's completion or wait for that turn without sending a prompt; during its resumed step, `getConversationHistory` for that agent is restricted to the same selected turn's response. Missing selected text fails rather than borrowing another turn. Other retry paths preserve their saved event.
